...
RUN mkdir -p /oppia/.git/hooks/ && cd /oppia/.git/hooks/ && if [ ! -L pre-commit ]; then ln -sfn /oppia/scripts/pre_commit_hook.py pre-commit; fi

RUN cd /oppia && ls && python2 -m scripts.start --no_browser --disable_host_checking --docker
RUN rm -rf /oppia

STOPSIGNAL SIGINT

ENTRYPOINT ["/oppia/entrypoint.sh"]
CMD ["python2", "-m", "scripts.start", "--no_browser", "--disable_host_checking", "--save_datastore"]
