...
/**
 * @fileoverview End-to-end testing utilities for the Button Choice
 * interaction.
 */
...
var expectInteractionDetailsToMatch = async function(
  ...
  var optionElements = elem.all(
    by.css('.protractor-test-Button-choice-option-container'));
  ...
  for (var i = 0; i < optionsCount; i++) {
    promises.push(await (await optionElements.get(i)).element(by.css(
      '.protractor-test-Button-choice-option')).getText());
  }
...
// 'answer' {String} is the text on the Button-choice item to select.
var submitAnswer = async function(elem, answer) {
  await elem.element(by.tagName('oppia-interactive-Button-choice-input')).
