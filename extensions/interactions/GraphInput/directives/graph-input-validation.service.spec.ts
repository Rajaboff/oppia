...
describe('GraphInputValidationService', () => {
  ...
  it('should validate the maximum number of vertices in the graph', () => {
    ...
    expect(warnings).toEqual([{
      ...
      message: 'Фигура, используемый при настройке, превышает поддерживаемый максимум ' +
  ...
  it('should validate the maximum number of vertices in the graph for an ' +
    ...
    expect(warnings).toEqual([{
      type: WARNING_TYPES.CRITICAL,
      message: 'Фигура, используемый в правиле 1 в группе 1, превышает поддерживаемые ' +
          'максимальное количество точек 10 для проверки изоморфизма.'
    }, {
      type: WARNING_TYPES.CRITICAL,
      message: 'Фигура, используемый в правиле 2 в группе 1, превышает поддерживаемые ' +
          'максимальное количество точек 10 для проверки изоморфизма.'
    }, {
      type: WARNING_TYPES.CRITICAL,
      message: 'Фигура, используемый в правиле 1 в группе 2, превышает поддерживаемые ' +
          'максимальное количество точек 10 для проверки изоморфизма.'
    }]);
  });
  ...
  it('should verify edge weight edit permissions make sense', () => {
    ...
    expect(warnings).toEqual([{
      type: WARNING_TYPES.CRITICAL,
      message: (
        'Учащийся не может редактировать значения отрезка невзвешенной фигуры.')
    }]);
  });
  ...
  it('should verify vertex label edit permissions make sense', () => {
    ...
    expect(warnings).toEqual([{
      type: WARNING_TYPES.CRITICAL,
      message: (
        'Учащийся не может редактировать названи точек для немаркированной фигуры..')
    }]);
  });
});
