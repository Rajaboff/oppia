...
## PUT /activity/access/{token}

Получение доступа к активности по токену.
Токен генерируется при завершении оплаты и отправляется пользователю на почту.

Payload передавать не нужно, только CSRF-токен
